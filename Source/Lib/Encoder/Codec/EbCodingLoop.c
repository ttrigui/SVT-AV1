...

{
...

    {
        ...
        if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
            ...
            //TODO: check component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||   component_mask ==      PICTURE_BUFFER_DESC_CHROMA_MASK
            if (blk_ptr->prediction_mode_flag == INTRA_MODE &&
                blk_ptr->prediction_unit_array->intra_chroma_mode == UV_CFL_PRED) {
...
#if ENCDEC_16BIT
    static void av1_encode_loop_8bit_input_16bit(
        ...
        uint32_t cb_qp,
        EbPictureBufferDesc * pred_samples, // no basis/offset
        EbPictureBufferDesc * pred_samples16bit, // no basis/offset
        EbPictureBufferDesc * coeff_samples_sb, // sb based
        EbPictureBufferDesc * residual16bit, // no basis/offset
        EbPictureBufferDesc * residual16bit_new, // no basis/offset
        EbPictureBufferDesc * transform16bit, // no basis/offset
        EbPictureBufferDesc * inverse_quant_buffer,
        int16_t * transform_scratch_buffer,
        ...
        const uint32_t input_luma_offset_16bit =
            context_ptr->blk_geom->tx_org_x[blk_ptr->tx_depth][context_ptr->txb_itr] +
            context_ptr->blk_geom->tx_org_y[blk_ptr->tx_depth][context_ptr->txb_itr] * SB_STRIDE_Y;
        const uint32_t input_cb_offset_16bit =
            ROUND_UV(context_ptr->blk_geom->tx_org_x[blk_ptr->tx_depth][context_ptr->txb_itr]) / 2 +
            ROUND_UV(context_ptr->blk_geom->tx_org_y[blk_ptr->tx_depth][context_ptr->txb_itr]) / 2 *
                SB_STRIDE_UV;
        const uint32_t input_cr_offset_16bit =
            ROUND_UV(context_ptr->blk_geom->tx_org_x[blk_ptr->tx_depth][context_ptr->txb_itr]) / 2 +
            ROUND_UV(context_ptr->blk_geom->tx_org_y[blk_ptr->tx_depth][context_ptr->txb_itr]) / 2 *
                SB_STRIDE_UV;
        const uint32_t pred_luma_offset_16bit =
            ((pred_samples16bit->origin_y + origin_y) * pred_samples16bit->stride_y) +
            (pred_samples16bit->origin_x + origin_x);
        const uint32_t pred_cb_offset_16bit =
            (((pred_samples16bit->origin_y + round_origin_y) >> 1) * pred_samples16bit->stride_cb) +
            ((pred_samples16bit->origin_x + round_origin_x) >> 1);
        const uint32_t pred_cr_offset_16bit =
            (((pred_samples16bit->origin_y + round_origin_y) >> 1) * pred_samples16bit->stride_cr) +
            ((pred_samples16bit->origin_x + round_origin_x) >> 1);
        ...
        if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
            component_mask == PICTURE_BUFFER_DESC_LUMA_MASK) {
            residual_kernel16bit(
                ((uint16_t *)input_samples16bit->buffer_y) + input_luma_offset_16bit,
                input_samples16bit->stride_y,
                ((uint16_t *)pred_samples16bit->buffer_y) + pred_luma_offset_16bit,
                pred_samples16bit->stride_y,
                ((int16_t *)residual16bit->buffer_y) + scratch_luma_offset,
                residual16bit->stride_y,
                context_ptr->blk_geom->tx_width[blk_ptr->tx_depth][context_ptr->txb_itr],
            ...
            blk_ptr->quantized_dc[0][context_ptr->txb_itr] =
                av1_quantize_inv_quantize_8bit_input_16bit(
                    sb_ptr->pcs_ptr,
                    context_ptr->md_context,
                    ((TranLow *)transform16bit->buffer_y) + coeff1d_offset,
                    NOT_USED_VALUE,
        ...
        if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
            ...
            if (blk_ptr->prediction_mode_flag == INTRA_MODE &&
                (context_ptr->evaluate_cfl_ep ||
                 blk_ptr->prediction_unit_array->intra_chroma_mode == UV_CFL_PRED)) {
                EbPictureBufferDesc *recon_samples16bit = pred_samples16bit;

                uint32_t recon_luma_offset16bit =
                    (recon_samples16bit->origin_y + round_origin_y) * recon_samples16bit->stride_y +
                    (recon_samples16bit->origin_x + round_origin_x);

                // Down sample Luma
                cfl_luma_subsampling_420_hbd_c(
                    ((uint16_t *)recon_samples16bit->buffer_y) + recon_luma_offset16bit,
                    recon_samples16bit->stride_y,
                    //pred_buf_q3_local,
                    context_ptr->md_context->pred_buf_q3,
                    context_ptr->blk_geom->bwidth_uv == context_ptr->blk_geom->bwidth
                        ? (context_ptr->blk_geom->bwidth_uv << 1)
                ...
                int height = context_ptr->blk_geom->bheight_uv == context_ptr->blk_geom->bheight
                                 ? (context_ptr->blk_geom->bheight_uv << 1)
                                 : context_ptr->blk_geom->bheight;
                int width = context_ptr->blk_geom->bwidth_uv == context_ptr->blk_geom->bwidth
                                ? (context_ptr->blk_geom->bwidth_uv << 1)
                                : context_ptr->blk_geom->bwidth;
                ...
                if (blk_ptr->prediction_unit_array->intra_chroma_mode == UV_CFL_PRED) {
                    int32_t alpha_q3 =
                        cfl_idx_to_alpha(blk_ptr->prediction_unit_array->cfl_alpha_idx,
                                         blk_ptr->prediction_unit_array->cfl_alpha_signs,
                                         CFL_PRED_U); // once for U, once for V
                    ...
                    eb_cfl_predict_hbd(
                        context_ptr->md_context->pred_buf_q3,
                        ((uint16_t *)pred_samples16bit->buffer_cb) + pred_cb_offset_16bit,
                        pred_samples16bit->stride_cb,
                        ((uint16_t *)pred_samples16bit->buffer_cb) + pred_cb_offset_16bit,
                        pred_samples16bit->stride_cb,
                        alpha_q3,
                    ...
                    eb_cfl_predict_hbd(
                        context_ptr->md_context->pred_buf_q3,
                        ((uint16_t *)pred_samples16bit->buffer_cr) + pred_cr_offset_16bit,
                        pred_samples16bit->stride_cr,
                        ((uint16_t *)pred_samples16bit->buffer_cr) + pred_cr_offset_16bit,
                        pred_samples16bit->stride_cr,
                        alpha_q3,
            ...
            //**********************************
            residual_kernel16bit(
                ((uint16_t *)input_samples16bit->buffer_cb) + input_cb_offset_16bit,
                input_samples16bit->stride_cb,
                ((uint16_t *)pred_samples16bit->buffer_cb) + pred_cb_offset_16bit,
                pred_samples16bit->stride_cb,
                ((int16_t *)residual16bit->buffer_cb) + scratch_cb_offset,
            ...
            residual_kernel16bit(
                ((uint16_t *)input_samples16bit->buffer_cr) + input_cr_offset_16bit,
                input_samples16bit->stride_cr,
                ((uint16_t *)pred_samples16bit->buffer_cr) + pred_cr_offset_16bit,
                pred_samples16bit->stride_cr,
                ((int16_t *)residual16bit->buffer_cr) + scratch_cr_offset,
            ...
            blk_ptr->quantized_dc[1][context_ptr->txb_itr] =
                av1_quantize_inv_quantize_8bit_input_16bit(
                    sb_ptr->pcs_ptr,
                    context_ptr->md_context,
                    ((TranLow *)transform16bit->buffer_cb) + context_ptr->coded_area_sb_uv,
            ...
            av1_estimate_transform(
                ...
                PLANE_TYPE_UV,
                DEFAULT_SHAPE);
            blk_ptr->quantized_dc[2][context_ptr->txb_itr] =
                av1_quantize_inv_quantize_8bit_input_16bit(
                    sb_ptr->pcs_ptr,
                    context_ptr->md_context,
                    ((TranLow *)transform16bit->buffer_cr) + context_ptr->coded_area_sb_uv,
...
*
**********************************************************/
    static void av1_encode_generate_recon(EncDecContext * context_ptr,
                                          uint32_t origin_x,
                                          uint32_t origin_y,
                                          EbPictureBufferDesc * pred_samples, // no basis/offset
                                          EbPictureBufferDesc * residual16bit, // no basis/offset
...
*
**********************************************************/
    static void av1_encode_generate_recon_16bit(
        EncDecContext * context_ptr,
        uint32_t origin_x,
        uint32_t origin_y,
        EbPictureBufferDesc * pred_samples, // no basis/offset
    ...
    void store16bit_input_src(EbPictureBufferDesc * input_sample16bit_buffer,
                              PictureControlSet * pcs_ptr,
                              uint32_t sb_x,
                              uint32_t sb_y,
                              uint32_t sb_w,
    ...
    void update_av1_mi_map(BlkStruct * blk_ptr,
                           uint32_t           blk_origin_x,
                           uint32_t           blk_origin_y,
                           const BlockGeom *  blk_geom,
                           PictureControlSet *pcs_ptr);

    void move_blk_data(PictureControlSet * pcs,
                       EncDecContext * context_ptr,
                       BlkStruct * src_cu,
                       BlkStruct * dst_cu);

    void perform_intra_coding_loop(PictureControlSet * pcs_ptr,
                                   SuperBlock * sb_ptr,
                                   uint32_t sb_addr,
                                   BlkStruct * blk_ptr,
        ...
        NeighborArrayUnit *ep_cb_recon_neighbor_array =
            is_16bit ? pcs_ptr->ep_cb_recon_neighbor_array16bit
                     : pcs_ptr->ep_cb_recon_neighbor_array;
        NeighborArrayUnit *ep_cr_recon_neighbor_array =
            is_16bit ? pcs_ptr->ep_cr_recon_neighbor_array16bit
        ...
        if (context_ptr->blk_geom->has_uv) {
            ...
            get_txb_ctx(pcs_ptr,
                        ...
                        context_ptr->blk_geom->bsize_uv,
                        context_ptr->blk_geom
                            ->txsize_uv[context_ptr->blk_ptr->tx_depth][context_ptr->txb_itr],
                        &context_ptr->md_context->cr_txb_skip_context,
                        &context_ptr->md_context->cr_dc_sign_context);
            ...
            if (is_16bit) {
                ...
                for (int32_t plane = 1; plane <= plane_end; ++plane) {
                    ...
                    if (plane == 1) {
                        if (blk_originy_uv != 0)
                            memcpy(top_neigh_array + 1,
                                   (uint16_t *)(ep_cb_recon_neighbor_array->top_array) +
                                       blk_originx_uv,
                                   context_ptr->blk_geom->bwidth_uv * 2 * sizeof(uint16_t));
                        if (blk_originx_uv != 0)
                            memcpy(left_neigh_array + 1,
                                   (uint16_t *)(ep_cb_recon_neighbor_array->left_array) +
                                       blk_originy_uv,
                        ...
                        if (blk_originy_uv != 0)
                            memcpy(top_neigh_array + 1,
                                   (uint16_t *)(ep_cr_recon_neighbor_array->top_array) +
                                       blk_originx_uv,
                                   context_ptr->blk_geom->bwidth_uv * 2 * sizeof(uint16_t));
                        if (blk_originx_uv != 0)
                            memcpy(left_neigh_array + 1,
                                   (uint16_t *)(ep_cr_recon_neighbor_array->left_array) +
                                       blk_originy_uv,
                                   context_ptr->blk_geom->bheight_uv * 2 * sizeof(uint16_t));
                        if (blk_originy_uv != 0 && blk_originx_uv != 0)
                            top_neigh_array[0] = left_neigh_array[0] =
                    ...
                    eb_av1_predict_intra_block_16bit(
                        ...
                        tx_size,
                        mode,
                        plane ? pu_ptr->angle_delta[PLANE_TYPE_UV]
                              : pu_ptr->angle_delta[PLANE_TYPE_Y],
                        0, //chroma
                        &blk_ptr->palette_info,
                        FILTER_INTRA_MODES,
...
#if ENCDEC_16BIT
    void perform_intra_coding_loop_16bit(PictureControlSet * pcs_ptr,
                                         SuperBlock * sb_ptr,
                                         uint32_t sb_addr,
                                         BlkStruct * blk_ptr,
                                         PredictionUnit * pu_ptr,
        ...
        EbBool              is_16bit = context_ptr->is_16bit || scs_ptr->static_config.is_16bitPipeline;
...
//        EbPictureBufferDesc *residual_buffer16bit = context_ptr->residual_buffer16bit;
//#endif
        EbPictureBufferDesc *residual_buffer           = context_ptr->residual_buffer;
        EbPictureBufferDesc *transform_buffer          = context_ptr->transform_buffer;
        EbPictureBufferDesc *inverse_quant_buffer      = context_ptr->inverse_quant_buffer;
        ...
        for (context_ptr->txb_itr = 0; context_ptr->txb_itr < tot_tu; context_ptr->txb_itr++) {
            ...
            if (is_16bit) {
                ...
                eb_av1_predict_intra_block_16bit(
#if ENCDEC_16BIT
                    context_ptr->is_16bit ? EB_10BIT : EB_8BIT,
#endif
                    &sb_ptr->tile_info,
                    ED_STAGE,
                    context_ptr->blk_geom,
            ...
            //            if (txb_origin_y != 0)
            //                memcpy(top_neigh_array + 1,
            //                       (uint16_t *)(ep_luma_recon_neighbor_array16bit->top_array) + txb_origin_x,
            //                       context_ptr->blk_geom->tx_width[blk_ptr->tx_depth][context_ptr->txb_itr] *
            //                           2 * sizeof(uint16_t));
            //            if (txb_origin_x != 0)
            //                memcpy(left_neigh_array + 1,
            //                       (uint16_t *)(ep_luma_recon_neighbor_array16bit->left_array) + txb_origin_y,
            //                       context_ptr->blk_geom->tx_height[blk_ptr->tx_depth][context_ptr->txb_itr] *
            //                           2 * sizeof(uint16_t));
            //            if (txb_origin_y != 0 && txb_origin_x != 0)
            //                top_neigh_array[0] = left_neigh_array[0] =
            //                    ((uint16_t *)(ep_luma_recon_neighbor_array16bit->top_left_array) +
            //                     MAX_PICTURE_HEIGHT_SIZE + txb_origin_x - txb_origin_y)[0];
            //
            //            mode = blk_ptr->pred_mode;
            //
            ...
            //#if ENCDEC_16BIT
            //        av1_encode_loop_8bit_input_16bit(pcs_ptr,
            //                                         context_ptr,
            //                                         sb_ptr,
            //                                         txb_origin_x,
            //                                         txb_origin_y,
            ...
            if (pcs_ptr->update_cdf) {
                ...
                av1_txb_estimate_coeff_bits(
                    ...
                    av1_enc_gen_recon_func_ptr[is_16bit](context_ptr,
                                                 ...
                                                 eobs[context_ptr->txb_itr]);
    //#if ENCDEC_16BIT
//            av1_encode_generate_recon_8bit_input_16bit(context_ptr,
//                                                       txb_origin_x,
//                                                       txb_origin_y,
//                                                       recon_buffer,
//                                                       inverse_quant_buffer,
...
//                dst_16bit[i + j * dst_stride_16bit] = (uint16_t)dst[i + j * dst_stride];
//            }
//        }
//#endif
#if ENCDEC_16BIT
            ...
            if (!context_ptr->is_16bit) {
                ...
                uint16_t *dst_16bit =
                    (uint16_t *)(recon_buffer->buffer_y) + pred_buf_x_offest +
                    recon_buffer->origin_x +
                    (pred_buf_y_offest + recon_buffer->origin_y) * recon_buffer->stride_y;
                int32_t dst_stride_16bit = recon_buffer->stride_y;
            ...
            /* if ((pred_buf_x_offest == 0) && (pred_buf_y_offest == 0)) {
            for (int j = 0; j < context_ptr->blk_geom->bheight; j++) {
                printf("\n");
                for (int i = 0; i < context_ptr->blk_geom->bwidth; i++) {
                    printf(" %d \t", dst_16bit[i + j * dst_stride_16bit]);
                }
            }
        }*/
#endif
...
//                        if (blk_originy_uv != 0)
//                            memcpy(top_neigh_array + 1,
//                                   (uint16_t *)(ep_cb_recon_neighbor_array16bit->top_array) +
//                                       blk_originx_uv,
//                                   context_ptr->blk_geom->bwidth_uv * 2 * sizeof(uint16_t));
//                        if (blk_originx_uv != 0)
//                            memcpy(left_neigh_array + 1,
//                                   (uint16_t *)(ep_cb_recon_neighbor_array16bit->left_array) +
//                                       blk_originy_uv,
//                                   context_ptr->blk_geom->bheight_uv * 2 * sizeof(uint16_t));
//                        if (blk_originy_uv != 0 && blk_originx_uv != 0)
//                            top_neigh_array[0] = left_neigh_array[0] =
//                                ((uint16_t *)(ep_cb_recon_neighbor_array16bit->top_left_array) +
//                                 MAX_PICTURE_HEIGHT_SIZE / 2 + blk_originx_uv - blk_originy_uv)[0];
//                    } else if (plane == 2) {
//                        if (blk_originy_uv != 0)
//                            memcpy(top_neigh_array + 1,
//                                   (uint16_t *)(ep_cr_recon_neighbor_array16bit->top_array) +
//                                       blk_originx_uv,
//                                   context_ptr->blk_geom->bwidth_uv * 2 * sizeof(uint16_t));
//                        if (blk_originx_uv != 0)
//                            memcpy(left_neigh_array + 1,
//                                   (uint16_t *)(ep_cr_recon_neighbor_array16bit->left_array) +
//                                       blk_originy_uv,
//                                   context_ptr->blk_geom->bheight_uv * 2 * sizeof(uint16_t));
//                        if (blk_originy_uv != 0 && blk_originx_uv != 0)
//                            top_neigh_array[0] = left_neigh_array[0] =
//                                ((uint16_t *)(ep_cr_recon_neighbor_array16bit->top_left_array) +
//                                 MAX_PICTURE_HEIGHT_SIZE / 2 + blk_originx_uv - blk_originy_uv)[0];
//                    }
//
//                    mode = (pu_ptr->intra_chroma_mode == UV_CFL_PRED)
...
//#endif
            ...
            if (!context_ptr->is_16bit) {
                ...
                //copy recon from 8bit to 16bit
                uint32_t pred_buf_x_offest = ((context_ptr->blk_origin_x >> 3) << 3) >> 1;
                uint32_t pred_buf_y_offest = ((context_ptr->blk_origin_y >> 3) << 3) >> 1;

                dst_16bit =
                    (uint16_t *)(recon_buffer->buffer_cb) + pred_buf_x_offest +
                    recon_buffer->origin_x / 2 +
                    (pred_buf_y_offest + recon_buffer->origin_y / 2) * recon_buffer->stride_cb;
                dst_stride_16bit = recon_buffer->stride_cb;

                dst = recon_buffer->buffer_cb + pred_buf_x_offest + recon_buffer->origin_x / 2 +
                      (pred_buf_y_offest + recon_buffer->origin_y / 2) * recon_buffer->stride_cb;
                ...
                dst_16bit =
                    (uint16_t *)(recon_buffer->buffer_cr) +
                    (pred_buf_x_offest + recon_buffer->origin_x / 2 +
                     (pred_buf_y_offest + recon_buffer->origin_y / 2) * recon_buffer->stride_cr);
                dst_stride_16bit = recon_buffer->stride_cr;
                dst = recon_buffer->buffer_cr + pred_buf_x_offest + recon_buffer->origin_x / 2 +
                      (pred_buf_y_offest + recon_buffer->origin_y / 2) * recon_buffer->stride_cr;
...
#define REFMVS_LIMIT ((1 << 12) - 1)
    ...
    void av1_copy_frame_mvs(PictureControlSet * pcs_ptr,
...
*
*******************************************/
    EB_EXTERN void av1_encode_pass(SequenceControlSet * scs_ptr,
                                   PictureControlSet * pcs_ptr,
                                   SuperBlock * sb_ptr,
                                   uint32_t sb_addr,
        ...
        NeighborArrayUnit *ep_mode_type_neighbor_array =
            pcs_ptr->ep_mode_type_neighbor_array[tile_idx];
        ...
        NeighborArrayUnit *ep_skip_flag_neighbor_array =
            pcs_ptr->ep_skip_flag_neighbor_array[tile_idx];
#else
        ...
        while (blk_it < scs_ptr->max_block_cnt) {
            ...
            UNUSED(blk_geom);
            sb_ptr->cu_partition_array[blk_it] =
                context_ptr->md_context->md_blk_arr_nsq[blk_it].part;
            if (pcs_ptr->update_cdf) {
                ...
                for (int32_t d1_itr = (int32_t)blk_it + offset_d1;
                    ...
                    if (blk_ptr->prediction_mode_flag == INTRA_MODE) {
                        ...
                        if (blk_ptr->av1xd->use_intrabc == 0) {
                            ...
                            generate_pu_intra_luma_neighbor_nodes(
                                blk_ptr,
                                context_ptr->blk_origin_x,
                                context_ptr->blk_origin_y,
                                BLOCK_SIZE_64,
                        ...
                        // Transform partitioning free patch (except the 128x128 case)
                            ...
                            {
                                uint32_t blk_originy_uv =
                                    (context_ptr->blk_origin_y >> 3 << 3) >> 1;
                                uint32_t blk_originx_uv =
                                    (context_ptr->blk_origin_x >> 3 << 3) >> 1;
...

                        {
                            ...
                            if (do_mc && pu_ptr->motion_mode != WARPED_CAUSAL) {
                                ...
                                for (int i = 0; i < blk_geom->bwidth; i++) {
                                    printf(" %d \t",
                                           (ref_pic_list0->buffer_y)[i + ref_pic_list0->origin_x +
...
EB_EXTERN void av1_encode_pass_16bit(SequenceControlSet *scs_ptr, PictureControlSet *pcs_ptr,
    ...
    NeighborArrayUnit *ep_luma_recon_neighbor_array_16bit_inter =
        pcs_ptr->ep_luma_recon_neighbor_array16bit;
    NeighborArrayUnit *ep_cb_recon_neighbor_array_16bit_inter =
        pcs_ptr->ep_cb_recon_neighbor_array16bit;
    NeighborArrayUnit *ep_cr_recon_neighbor_array_16bit_inter =
        pcs_ptr->ep_cr_recon_neighbor_array16bit;
#endif
            ...
            for (int32_t d1_itr = (int32_t)blk_it + offset_d1;
                ...
                if (blk_ptr->prediction_mode_flag == INTRA_MODE) {
                    ...
                    if (blk_ptr->av1xd->use_intrabc == 0) {
                        ...
                        generate_pu_intra_luma_neighbor_nodes(blk_ptr,
                                                              ...
                                                              ep_mode_type_neighbor_array);
#if ENCDEC_16BIT
                        perform_intra_coding_loop_16bit(
                            pcs_ptr, sb_ptr, sb_addr, blk_ptr, pu_ptr, context_ptr);
                ...
                // Inter
...

                    {
                        ...
                        if (do_mc && pu_ptr->motion_mode != WARPED_CAUSAL) {
                            ...
                            for (int j = 0; j < blk_geom->bheight; j++) {
                                for (int i = 0; i < blk_geom->bwidth; i++) {
                                    /* if ((ref_pic_list0->buffer_y)[i + ref_pic_list0->origin_x +
                                        ...
                                        printf(" PRED Y");*/
                                    if (((uint16_t *)recon_buffer_16bit_inter
                                             ->buffer_y)[recon_buffer_16bit_inter->origin_x + i +
                                                         context_ptr->blk_origin_x +
                                                         (recon_buffer_16bit_inter->origin_y +
                                                          context_ptr->blk_origin_y + j) *
                                                             recon_buffer_16bit_inter->stride_y] !=
                                        (recon_buffer->buffer_y)[recon_buffer->origin_x + i +
                                                                 context_ptr->blk_origin_x +
                                                                 (recon_buffer->origin_y + j +
                                                                  context_ptr->blk_origin_y) *
                                                                     recon_buffer->stride_y])
                                        printf(" RECON Y");

                                    if ((recon_buffer->buffer_cb)
                                            [(recon_buffer->origin_x + i +
                                              ((context_ptr->blk_origin_x >> 3) << 3)) /
                                                 2 +
                                             (recon_buffer->origin_y + j +
                                              ((context_ptr->blk_origin_y >> 3) << 3)) /
                                                 2 * recon_buffer->stride_cb] !=
                                        ((uint16_t *)recon_buffer_16bit_inter->buffer_cb)
                                            [(recon_buffer_16bit_inter->origin_x +
                                              ((context_ptr->blk_origin_x >> 3) << 3)) /
                                                 2 +
                                             i +
                                             (((recon_buffer_16bit_inter->origin_y +
                                               ...
                                               2) +
                                              j) *
                                                 recon_buffer_16bit_inter->stride_cb])
                                        printf(" RECON CB");
                                    ...
                                    if ((recon_buffer->buffer_cr)
                                            [(recon_buffer->origin_x + i +
                                             ...
                                             (recon_buffer->origin_y + j +
                                              ((context_ptr->blk_origin_y >> 3) << 3)) /
                                                 2 * recon_buffer->stride_cr] !=
                                        ((uint16_t *)recon_buffer_16bit_inter->buffer_cr)
                                            [(recon_buffer_16bit_inter->origin_x +
                                              ((context_ptr->blk_origin_x >> 3) << 3)) /
                                                 2 +
                                             i +
                                             (((recon_buffer_16bit_inter->origin_y +
                                               ...
                                               2) +
                                              j) *
                                                 recon_buffer_16bit_inter->stride_cr])
                                        printf(" RECON CR");
                                }
                            }
                            ...
                            av1_inter_prediction_ldb_in_hbd(
                                ...
                                ep_cb_recon_neighbor_array,
                                ep_cr_recon_neighbor_array,
                                ep_luma_recon_neighbor_array_16bit_inter,
                                ep_cb_recon_neighbor_array_16bit_inter,
                                ep_cr_recon_neighbor_array_16bit_inter,
                                blk_ptr->is_interintra_used,
                                blk_ptr->interintra_mode,
                                blk_ptr->use_wedge_interintra,
                                blk_ptr->interintra_wedge_index,
                    ...
                    if (do_recon) {
                        ...
                        uint16_t *dst_16bit =
                            (uint16_t *)(recon_buffer_16bit_inter->buffer_y) + pred_buf_x_offest +
                            recon_buffer_16bit_inter->origin_x +
                            (pred_buf_y_offest + recon_buffer_16bit_inter->origin_y) *
                                recon_buffer_16bit_inter->stride_y;
                        int32_t dst_stride_16bit = recon_buffer_16bit_inter->stride_y;
                        ...
                        dst_16bit = (uint16_t *)(recon_buffer_16bit_inter->buffer_cb) +
                                    pred_buf_x_offest + recon_buffer_16bit_inter->origin_x / 2 +
                                    (pred_buf_y_offest + recon_buffer_16bit_inter->origin_y / 2) *
                                        recon_buffer_16bit_inter->stride_cb;
                        dst_stride_16bit = recon_buffer_16bit_inter->stride_cb;
                        ...
                        dst_16bit = (uint16_t *)(recon_buffer_16bit_inter->buffer_cr) +
                                    (pred_buf_x_offest + recon_buffer_16bit_inter->origin_x / 2 +
                                     (pred_buf_y_offest + recon_buffer_16bit_inter->origin_y / 2) *
                                         recon_buffer_16bit_inter->stride_cr);
                        dst_stride_16bit = recon_buffer_16bit_inter->stride_cr;
